<div>
    <div class="bg-white">
        <div class="mx-auto grid max-w-3xl grid-cols-1 gap-x-8 gap-y-16 px-4 py-16 sm:grid-cols-2 sm:px-6 xl:max-w-none xl:grid-cols-3 xl:px-8 2xl:grid-cols-4">
            ...
            @foreach($calendarYear as $keyMonth=> $month)


                <section class="text-center">
                    <h2 class="text-sm font-semibold text-gray-900">January{{$keyMonth}}</h2>
                    ...
                    <div class="isolate mt-2 grid grid-cols-7 gap-px rounded-lg bg-gray-200 text-sm shadow ring-1 ring-gray-200">
                        @foreach($month as $day)
                            ...
                            {
                                case 1: $currentBorder ='rounded-tl-lg';break;

                            }

                            $color = isset($day[4])&&$day[4] ?'bg-indigo-600 font-semibold text-white':''
                            @endphp

                            @if($keyMonth === intval($day[2]))
                                <button type="button" class="bg-white py-1.5 hover:bg-gray-100 focus:z-10">
                                                                <time datetime="2022-01-14" class="mx-auto flex h-7 w-7 items-center justify-center rounded-full {{$color}}">{{$day[1]}}</time>
                                                            </button>
                                @else
                                <button type="button" class="rounded-tl-lg bg-gray-50 py-1.5 text-gray-400 hover:bg-gray-100 focus:z-10">

                                    <time datetime="2021-12-27" class="mx-auto flex h-7 w-7 items-center justify-center rounded-full {{$color}}">{{$day[1]}}</time>
                                </button>
                            @endif
